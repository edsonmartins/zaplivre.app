...
pub fn encrypt_for_storage(key: &[u8; 32], plaintext: &[u8]) -> Result<Vec<u8>> {
    ...
    let ciphertext = cipher
        ...
        .map_err(|e| ZapLivreError::Crypto(format!("Storage encryption failed: {}", e)))?;
    ...
    bincode::serialize(&envelope)
        .map_err(|e| ZapLivreError::Crypto(format!("Storage encrypt serialize failed: {}", e)))
}

pub fn decrypt_for_storage(key: &[u8; 32], blob: &[u8]) -> Result<Vec<u8>> {
    let envelope: StorageEnvelope = bincode::deserialize(blob)
        .map_err(|e| ZapLivreError::Crypto(format!("Storage decrypt deserialize failed: {}", e)))?;
    let cipher = Aes256Gcm::new(key.into());
    let nonce = Nonce::from_slice(&envelope.nonce);
    let plaintext = cipher
        .decrypt(nonce, envelope.ciphertext.as_ref())
        .map_err(|e| ZapLivreError::Crypto(format!("Storage decryption failed: {}", e)))?;
    Ok(plaintext)
}
