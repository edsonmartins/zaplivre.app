 ...
 * - Gerenciar lifecycle do ZapLivreClient
 */
class ZapLivreService : Service() {

    companion object {
        private const val TAG = "ZapLivreService"
        private const val NOTIFICATION_ID = 1
        private const val CHANNEL_ID = "zaplivre_service_channel"
        private const val CALL_CHANNEL_ID = "zaplivre_incoming_calls"
        private const val CALL_NOTIFICATION_ID = 2

        fun start(context: Context) {
            val intent = Intent(context, ZapLivreService::class.java)
    ...
    override fun onCreate() {
        ...
        serviceScope.launch {
            if (!ZapLivreClientWrapper.isClientReady()) {
                ...
                val hasIdentity = !com.zaplivre.core.AndroidIdentityStore
                ...
                Log.i(TAG, "Initializing ZapLivreClient from service")
                // MESSAGE_STORE_URL/SIGNALING_SERVER_URL são configuradas em
                // ZapLivreApplication.onCreate, antes de qualquer initialize()
            ...
            ZapLivreClientWrapper.listenOn("/ip4/0.0.0.0/tcp/0")

            // Bootstrap (conectar a nodes conhecidos)
            Log.i(TAG, "Starting bootstrap")
            ZapLivreClientWrapper.bootstrap()
    ...
    private fun observeIncomingCalls() {
        serviceScope.launch {
            ZapLivreClientWrapper.incomingCall.collect { call ->
                if (call == null) {
                ...
                val intent = Intent(this@ZapLivreService, com.zaplivre.MainActivity::class.java).apply {
                    flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP
                }
                val fullScreenIntent = PendingIntent.getActivity(
                    this@ZapLivreService,
                    1,
                    intent,
                    PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
                )

                val notification = NotificationCompat.Builder(this@ZapLivreService, CALL_CHANNEL_ID)
    ...
    private fun createNotification(connectedPeers: UInt): Notification {
        ...
        val intent = Intent(this, com.zaplivre.MainActivity::class.java).apply {
            flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP
        }
    ...
    private fun startPeerMonitoring() {
        monitoringJob = serviceScope.launch {
            while (isActive) {
                try {
                    val count = ZapLivreClientWrapper.getConnectedPeersCount()
