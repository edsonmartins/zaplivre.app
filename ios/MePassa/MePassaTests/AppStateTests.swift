...
//
//  Unit tests for AppState transitions using a fake MePassaCoreProtocol,
...
final class FakeCore: MePassaCoreProtocol {
...
final class AppStateTests: XCTestCase {
    ...
    func testCoreReportedIdentityTransitionsToAuthenticated() {
        ...
        // Mirror MePassaApp.initializeMePassaCore: initialize the core, then
    ...
    func testCoreInitializationErrorStaysUnauthenticated() {
        core.initializeError = MePassaCoreError.storageError("disk full")

        let initDone = expectation(description: "core init attempted")
        Task { [core = core!] in
            // Mirror MePassaApp.initializeMePassaCore's error path: on throw,
            // no login happens.
            do {
                try await core.initialize()
    ...
    func testLoadConversationsFailureLeavesStateUnchanged() {
        core.listConversationsError = MePassaCoreError.networkError("offline")
