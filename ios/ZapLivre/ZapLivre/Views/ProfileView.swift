...
struct ProfileView: View {
    ...
    private func loadPeerId() {
        localPeerId = ZapLivreCore.shared.localPeerId ?? ""
    }

    private func exportIdentity() {
        Task {
            do {
                exportData = try await ZapLivreCore.shared.exportIdentity()
    ...
    private func exportPrekeyBundle() {
        Task {
            do {
                exportData = try await ZapLivreCore.shared.exportPrekeyBundle()
                showExportSheet = true
            } catch {
                exportErrorMessage = error.localizedDescription
                showExportError = true
