...
struct VideoCallScreen: View {
    ...
    private func toggleVideo() {
        ...
        if videoEnabled {
            ...
            Task {
                do {
                    try await ZapLivreCore.shared.enableVideo(callId: callId, codec: .h264)
                } catch {
            ...
            Task {
                do {
                    try await ZapLivreCore.shared.disableVideo(callId: callId)
                } catch {
    ...
    private func toggleMute() {
        ...
        Task {
            do {
                try await ZapLivreCore.shared.toggleMute(callId: callId)
            } catch {
    ...
    private func switchCamera() {
        ...
        Task {
            do {
                try await ZapLivreCore.shared.switchCamera(callId: callId)
            } catch {
    ...
    private func hangup() {
        stopVideo()
        Task {
            try? await ZapLivreCore.shared.hangupCall(callId: callId)
        }
        onHangup()
    }
    ...
    private func startVideo() {
        cameraManager.startCaptureEncoded { frame, width, height in
            Task {
                do {
                    try await ZapLivreCore.shared.sendVideoFrame(
                        callId: self.callId,
                        frameData: frame,
                        width: width,
                        height: height
...
struct RemoteVideoView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UIView {
        ...
        Task {
            do {
                try await ZapLivreCore.shared.registerVideoFrameCallback(context.coordinator.videoHandler)
